...
We will see a quick setup to configure the ``hass`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``hass`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``hass`` formulas under ``/srv/salt/hass/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/hass/parameters/dns:domain/
    ln -s dns:domain /srv/salt/hass/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/hass/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/hass/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``hass`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``hass`` formulas under ``/srv/salt/hass/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/hass/parameters/roles
...
We create a configuration for the role ``hass/server`` in ``/srv/salt/hass/parameters/roles/hass/server.yaml``:
...
We create another configuration for the role ``hass/client`` in ``/srv/salt/hass/parameters/roles/hass/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles hass/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles hass/client
...
.. code-block:: console
    ...
    mv /srv/salt/hass/parameters/dns:domain/  '/srv/salt/hass/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-hass-as-json:
      test.nop:
        - name: {{ hass | json }}
...
Here is an example based on `template-formula/hass/config/file.sls`_:
    ...
    hass-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='hass-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ hass.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``hass`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ hass['winner'] }}
...
.. _template-formula/hass/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/hass/config/file.sls
